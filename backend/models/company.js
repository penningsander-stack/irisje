...
router.get("/companyContext/:companySlug", async (req, res) => {
  try {
    ...
    // 1. Bronbedrijf ophalen (ACTIEF bedrijf)
    ...
    const { categories, specialties, city } = sourceCompany;

    if (!Array.isArray(categories) || categories.length === 0 || !city) {
      return res.status(400).json({
        ok: false,
    ...
    // 2. Kandidaten ophalen (zelfde categorie, andere bedrijven)
    ...
    // 3. Scoren: city eerst, daarna specialism-overlap
