...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (!res.ok) throw new Error(data.message || 'Fout bij ophalen bedrijfsinfo');

    const email = data.email || 'Onbekend';
    const bedrijf = data.company ? data.company.name : 'Onbekend';
    const categorie = data.company ? data.company.category : 'Onbekend';
